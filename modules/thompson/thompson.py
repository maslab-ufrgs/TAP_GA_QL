...
class Thompson:
    def __init__(self, experiment, drivers, k):
        ...
        self.episode = 0
        self.parameter_update_interval = 5 ## interval between updates on the parameters for the distributions
            ...
            if (self.episode % self.parameter_update_interval == 0) or (self.episode == self.num_actions * 2):
                for dInx in range(self.num_drivers):
                    for i in range(self.num_actions):
                        ...
                        decay = 0.9
                        weights = []
                        for x in self.observations[dInx][i][1]:
                            weights.append(decay**(self.episode - x) )

                        avg, std = calculate_std_avg(self.observations[dInx][i][0],weights)
                        self.sd[dInx][i] = std + epsilon
                        self.av[dInx][i] = avg
